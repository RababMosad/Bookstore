<div>
        ...
        <x-slot name="form">
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-jet-label for="permissions" value="{{ __('اذن') }}" />
                                ...
                                <div class="flex items-center">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ __('آخر أستخدام') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
    ...
    <x-jet-dialog-modal wire:model="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ __('يرجى نسخ رمز API الجديد الخاص بك. من أجل أمانك ، لن يتم عرضها مرة أخرى.') }}
            </div>
    ...
    <x-jet-confirmation-modal wire:model="confirmingApiTokenDeletion">
        ...
        <x-slot name="content">
            {{ __('هل أنت متأكد من رغبتك في حذف هذا الرمز المميز لواجهة برمجة التطبيقات؟') }}
        </x-slot>
